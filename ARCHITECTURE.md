...
*Master schematic — the definitive visual reference for the Smart Claude Memory v2.5.x production baseline (v2.5.0 migrates the data layer off Supabase onto a self-hosted plain PostgreSQL 17 + pgvector database — `src/db/pg-adapter.ts` (`pg.Pool`-backed) now sits behind the unchanged `src/supabase.ts` doorway; ~127 call sites + the full tool surface are untouched, only the backend changed — see §7. Prior: v2.4.0 four Session-48 epics — Phase 1 drag-drop Backlog Kanban + `PATCH /api/backlog/:id`, Phase 2 GLOBAL vault export/import via `export_global_vault` / `import_global_vault` (§4.3.1), and the Agentic Superpowers MVP `fetch_url` / `research_url` + SSRF guard + `html-to-text` — see §4.14; v2.3.0 M8.3 Semantic Clustering — see §4.13; v2.2.2 Agentic Resource Manager — see §4.12; v2.3.1 Active Backlog Kanban + `/api/backlog` + Epic G `file_watcher` daemon + migration `024`).*
...
1. **MCP server**: `mcpServers.smart-claude-memory` declares `command: "node"` with args `["${CLAUDE_PLUGIN_ROOT}/dist/index.js"]`. Claude Code's plugin loader resolves `${CLAUDE_PLUGIN_ROOT}` to the installed plugin directory at runtime. The SCM env vars (`SUPABASE_DB_URL` and/or `SUPABASE_POOLER_URL` — the Postgres connection strings; at least one required — plus `OLLAMA_HOST`, `OLLAMA_EMBED_MODEL`, `MEMORY_ROOTS`, `EMBED_DIM`) pass through from the host shell, with sensible defaults on the three Ollama/embed knobs. As of v2.5.0 the removed REST vars `SUPABASE_URL` / `SUPABASE_SECRET_KEY` are no longer read; the `SUPABASE_` prefix on the surviving DB-URL vars is retained for now (rename is future work).
...
On a freshly installed plugin against an empty PostgreSQL 17 + pgvector database (e.g. the bundled `infra/plain-pg/docker-compose.yml` on host port `5433`), the first `init_project()` call:

1. Runs the existing readiness checks (env, hook registration, MCP wiring, `dist/` build, Core 3 audit).
2. Opens a fresh `pg.Client` against the configured Postgres URL (`SUPABASE_POOLER_URL` when set, else `SUPABASE_DB_URL`), calls `applyPendingMigrations()` from `src/lib/migrations.ts`. The helper diffs `scripts/*.sql` against the `schema_migrations(filename PK, sha256, applied_at)` ledger and applies pending files transactionally — one `BEGIN/COMMIT` per migration, `ROLLBACK` on any failure.
