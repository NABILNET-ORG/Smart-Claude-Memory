...
> **Note (post-Session-12 correction).** A "Continuation 2" section was briefly appended here in commit `d429fe3` describing what turned out to be Session 12 work. It has been re-homed in [SESSION-12-REPORT.md](SESSION-12-REPORT.md). Session 11 ends at the close above; the `SCM-S11-D7` ID originally used during that work is superseded by `SCM-S12-D1`.
